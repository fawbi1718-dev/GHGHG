...
export default function RequiredOrganizationProfileModal({
  ...
  return (
    ...
    >
      ...
      >
        {/* Modal Top Header */}
        <div className="bg-brand-800 text-white p-5 sm:p-6 relative shrink-0">
          <div className="flex items-center justify-between gap-3 mb-3">
            <div className="flex items-center gap-2.5">
              <div className="w-10 h-10 rounded-lg bg-white/15 border border-white/20 flex items-center justify-center text-white shadow-inner">
                {isWarehouse ? <Building2 className="w-5 h-5" /> : <Store className="w-5 h-5" />}
              </div>
              <div>
                <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-brand-200 block">
                  {isWarehouse 
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4">
          ...
          {errorMessage && (
            <div className="p-3 bg-rose-50 border border-rose-200 rounded-lg text-rose-800 text-xs font-semibold flex items-start gap-2.5">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
              {isWarehouse ? <Building2 className="w-3.5 h-3.5 text-brand-700" /> : <Store className="w-3.5 h-3.5 text-brand-700" />}
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 bg-[#F4F7F5] rounded-xl border border-brand-100 text-xs font-bold text-slate-900 focus:bg-white focus:border-brand-500 focus:ring-2 focus:ring-brand-100 focus:outline-none transition-all"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full px-3 py-2.5 bg-[#F4F7F5] rounded-xl border border-brand-100 text-xs font-bold text-slate-900 focus:bg-white focus:border-brand-500 focus:outline-none transition-all"
              >
                {SYRIAN_CITIES.map((c) => (
          ...
          <div className="p-3 bg-brand-50/70 border border-brand-200/70 rounded-lg flex items-center gap-2.5 text-[11px] text-brand-900 font-medium">
          ...
          <div className="pt-3 border-t border-slate-100 flex flex-col-reverse sm:flex-row sm:items-center justify-between gap-3">
            ...
            <button
              type="submit"
              disabled={isSubmitting || authLoading}
              className="px-6 py-2.5 bg-brand-700 hover:bg-brand-800 active:scale-98 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-brand-700/20 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
            >
              {isSubmitting ? (
                <>
